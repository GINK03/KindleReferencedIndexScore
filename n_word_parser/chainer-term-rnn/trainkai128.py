...
last_data_dir = args.data_dir.split('/').pop()
...
#    if (i + 1) % 10000 == 0:
    if (i + 1) % 300 == 0:
        fn = ('%s/charrnn_%s_%d_epoch_%.2f.chainermodel' % (args.checkpoint_dir, last_data_dir, n_units, float(i)/jump))
        pickle.dump(copy.deepcopy(model).to_cpu(), open(fn, 'wb'))
        pickle.dump(copy.deepcopy(model).to_cpu(), open('%s/latest_%s_%d.chainermodel'%(args.checkpoint_dir, last_data_dir, n_units), 'wb'))
