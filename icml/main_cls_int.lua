...
opt = {
   ...
   init_g = '/home/gimpei/icml2016/model/cub_v2_nc4_cls0.5_int1.0_ngf128_ndf64_G_60',
   init_d = '/home/gimpei/icml2016/model/cub_v2_nc4_cls0.5_int1.0_ngf128_ndf64_D_60',
   use_cudnn = 0,
}
...
if opt.init_g == '' then
  ...
  print('netG model, try to load ! ' .. opt.init_g)
  netG = torch.load(opt.init_g)
  print('netG model was loaded! ')
end
...
for epoch = 1, opt.niter do
  ...
  for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
    ...
    if ((i-1) / opt.batchSize) % opt.print_every == 0 then
      print(('[%d][%d/%d] T:%.3f  DT:%.3f lr: %.4g '
                .. '  Err_G: %.4f  Err_D: %.4f Err_W: %.4f Elapsed:%.3f' 
                .. ' speed_per_epoch ' 
                .. tostring(elapsed_tm:time().real/epoch ) .. "sec"
                .. ' epoch is ' 
                .. tostring(epoch) 
                .. ' opt.niter ' 
                .. tostring(opt.niter)):format(
              epoch, ((i-1) / opt.batchSize),
              math.floor(math.min(data:size(), opt.ntrain) / opt.batchSize),
              tm:time().real, data_tm:time().real,
              optimStateG.learningRate,
              errG and errG or -1, errD and errD or -1,
              errW and errW or -1, elapsed_tm:time().real
              ))
  ...
  if epoch % 10 == 0 then --epoch % opt.save_every == 0 then
    paths.mkdir(opt.checkpoint_dir)
    -- torch.save(opt.checkpoint_dir .. '/' .. opt.name .. '_G_' .. epoch .. '.t7', netG)
    -- torch.save(opt.checkpoint_dir .. '/' .. opt.name .. '_D_' .. epoch .. '.t7', netD)
    -- torch.save(opt.checkpoint_dir .. '/' .. opt.name .. '_OPT_' .. epoch .. '.t7', opt)
    torch.save('model/' .. opt.name .. '_G_' .. epoch , netG)
    torch.save('model/' .. opt.name .. '_D_' .. epoch , netD)
    torch.save('model/' .. opt.name .. '_OPT_' .. epoch , opt)
