...
def isValidURL(link, prefix):
  ...
  if 'image' in link or '.jpg' in link:
      return False
      ...
      while links != set():
          ...
          for l in ls:
            if isValidURL(l, prefix):
                ...
                print "error", l
          print link, title, "パースしたよ, 残リンク数", len(links), ls, contents0
          contents = contents0
...
if '-d' in sys.argv:
  for url, contents in db:
    if 'iemo.jp' in url and '.jpg' not in url:
      for line in filter(lambda x: "。" in x, contents.split(' ') ):
          print line
