...
def html_adhoc_fetcher(url):
    ...
    contents0_text = (lambda x:x.text.encode('utf-8') if x != None else "" ) \
            ( soup.find('div', {'id': 'CONTENTS_MAIN'}) )
        
    #print "c0t", soup.find('div', {'id': 'CONTENTS_MAIN'})
    links = set([a['href'] for a in soup.find_all('a', href=True)])
    return title, contents0_text, links
html = open('./nikkei.html').read()
soup = bs4.BeautifulSoup(html, "html.parser")
import plyvel
import sys
db = plyvel.DB('nikkei.ldb', create_if_missing=True) 
...
if '-c' in sys.argv:
  ...
  try:
    links = set()
    links = P.loads(open('already_parsed_nikkei.pkl').read())
  except:
    pass
  ...
  while True:
    ...
    if len(link) <= 2:
        continue
    if link[0]  == '/':
        link = "http://www.nikkei.co.jp" + link
    print link
    if "www.nikkei.co.jp" not in link:
    ...
    if db.get(str(link)) == None:
      ...
      #while CT() > 1500 :
        #time.sleep(0.001) 
      #  print 'now spooling threaing ', CT()
      #  pass
      import cPickle as P
      import random
      if random.random() > 0.9:
        open('already_parsed_nikkei.pkl', 'w').write(P.dumps(links))
# ダンプモード
if '-d' in sys.argv:
