...
class DCGAN(object):
    def __init__(self, sess, image_size=108, is_crop=True,
                 batch_size=100, sample_size = 100, output_size=100,
    ...
    def build_model(self):
        if self.y_dim:
            print('[INFO] self.y_dim is defined as ', self.y_dim)
            self.y= tf.placeholder(tf.float32, [self.batch_size, self.y_dim], name='y')
        else:
            print('[INFO] self.y_dim is NOT defined as ', self.y_dim)
        ...
        print('[INFO] self.z is defined as', self.z, 'self.z_dim', self.z_dim)
        ...
        print('[INOF] Tensor size of d_loss is ', self.d_loss.get_shape())
    ...
    def train(self, config):
        ...
        gadam   = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1)
        print('[INFO][InTrain] self.g_vars  are', self.g_vars)
        for var in self.g_vars:
          print var.name, var, var.get_shape()
        print('[INFO][InTrain] self.g_loss of shapes are', self.g_loss)
        g_optim = gadam.minimize(self.g_loss, var_list=self.g_vars)
        ...
        if self.load(self.checkpoint_dir):
            ...
            print(" [!] Load failed...")
        print('any 1', config.epoch, os.path.join("./data", config.dataset, "*.jpg"))
        #print('any 2', glob(os.path.join("./data", config.dataset, "*.jpg")) )
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if np.mod(counter, 5) == 1:
                    print('[Dump Sample] make random seed, will dump picture')
                    sample_files = data[0:self.sample_size]
                    sample = [get_image(sample_file, self.image_size, is_crop=self.is_crop, resize_w=self.output_size, is_grayscale = self.is_grayscale) for sample_file in sample_files]
                    if (self.is_grayscale):
                      sample_images = np.array(sample).astype(np.float32)[:, :, :, None]
                    ...
                    save_images(samples, [10, 10],
                                './samples/train_micro_{:02d}_{:04d}.jpg'.format(epoch, idx))
                    print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss))
                    # 学習データを保存
                    self.save(config.checkpoint_dir, counter)

                if np.mod(counter, 500) == 3:
                    self.save(config.checkpoint_dir, counter)
    ...
    def save(self, checkpoint_dir, step):
        print('[Dump] Saver called')
        model_name = "DCGAN.model." + str(step)
    ...
    def load(self, checkpoint_dir, config=None):
        print(" [*] Reading checkpoints...")

        model_dir = "%s_%s_%s" % (self.dataset_name, self.batch_size, self.output_size)
        checkpoint_dir = os.path.join(checkpoint_dir, model_dir)
        print(" ckeckpoint dir is", checkpoint_dir)
        ckpt = tf.train.get_checkpoint_state(checkpoint_dir)
        
        #ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        print("retore 1", os.path.join(checkpoint_dir,"DCGAN.model.296-296" ))#, ckpt_name))
        #if ckpt and ckpt.model_checkpoint_path:
        print("retore 2")
        #ckpt_name = os.path.basename(ckpt.model_checkpoint_path)
        #self.saver.restore(self.sess, os.path.join(checkpoint_dir, ckpt_name))
        try:
          self.saver.restore(self.sess, os.path.join(checkpoint_dir, "DCGAN.model.66-66"))
          print("model restored!", os.path.join(checkpoint_dir, "DCGAN.model"))
        except:
          print("model missingd!")
          return False
        # add data loading function
        if config != None:
          data = glob(os.path.join("./data", config.dataset, "*.jpg"))
          np.random.shuffle(data)
          print('[Dump Sample] make random seed, will dump picture image_size', self.image_size, "is_crop", self.is_crop)
          sample_files = data[0:self.sample_size]
          sample = [get_image(sample_file, self.image_size, is_crop=self.is_crop, resize_w=self.output_size, is_grayscale = self.is_grayscale) for sample_file in sample_files]
          if (self.is_grayscale):
            sample_images = np.array(sample).astype(np.float32)
            ...
            print('normal mode', self.sample_size)
            sample_images = np.array(sample).astype(np.float32)
          sample_z = np.random.uniform(-1, 1, size=(self.sample_size , self.z_dim))
          #sample_z = np.random.uniform(0, 0, size=(self.sample_size , self.z_dim))
          samples, d_loss, g_loss = self.sess.run(
              [self.sampler, self.d_loss, self.g_loss],
              feed_dict={self.z: sample_z, self.images: sample_images}
          )
          save_images(samples, [8, 8],
                      './samples/train_resumed_{:20d}_{:04d}.jpg'.format(int(time.time()), 0))
          print("[Sample] d_loss: %.8f, g_loss: %.8f" % (d_loss, g_loss))
        return True
