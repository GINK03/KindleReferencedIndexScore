...
class Solver(object):
    ...
    def __init__(self, model, \
		 ...
		 pretrained_model='model/source_model-20000', \
		 test_model='model/dtn-1800'):
    ...
    def load_source(self, image_dir, split='train'):
        ...
        #source = scipy.io.loadmat(image_dir)
        images = np.transpose(np.array( [np.asarray(Image.open(x)) for x in glob.glob('./celeba/male/*')] ), [0, 1, 2, 3]) / 127.5 - 1
        """
	images.shape = (None, 32, 32, 3)
	"""
        #images = np.transpose(source['X'], [3, 0, 1, 2]) / 127.5 - 1
        #labels = source['y'].reshape(-1)
        labels = np.array( [int(random.random()*10) for _ in range(images.shape[0]) ] )
        print(images.shape)
        """
	labels = (None, 9)
	"""
    ...
    def load_target(self, image_dir, split='train'):
        ...
        images = np.transpose(np.array( [np.asarray(Image.open(x)) for x in glob.glob('./celeba/female/*')] ), [0, 1, 2, 3]) / 127.5 - 1
        #labels = target['y']
        labels = np.array( [int(random.random()*10) for _ in range(images.shape[0]) ] )
    ...
    def pretrain(self):
        ...
        test_images,  test_labels = self.load_source(self.source_dir, split='test')
    ...
    def train(self):
        # load svhn dataset
        source_images, _ = self.load_source(self.source_dir, split='train')
        target_images, _ = self.load_target(self.target_dir, split='train')
        print("a", target_images.shape )
        # build a graph
        model = self.model
        model.build_model()
    ...
    def eval(self):
        ...
        # load svhn dataset
        source_images, _ = self.load_source(self.source_dir)
