...
#    if (i + 1) % 10000 == 0:
    if (i + 1) % 500 == 0:
        fn = ('%s/%d_charrnn_epoch_%.2f.chainermodel' % (args.checkpoint_dir, n_units , float(i)/jump))
        pickle.dump(copy.deepcopy(model).to_cpu(), open(fn, 'wb'))
        pickle.dump(copy.deepcopy(model).to_cpu(), open('%s/%d_latest.chainermodel'%(args.checkpoint_dir, n_units), 'wb'))
